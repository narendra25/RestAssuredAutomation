...
public class MeetingCountApi extends SpecUtils  {
	Response response;
	...
	@Given("add the MeetingCount payload with {string} {string} {string}")
	public void add_the_assignee_payload_with(String userPrincipalName, String startDate,String endDate) throws IOException {
		reqspec = given().spec(requestSpecification()).header("x-access", accessToken)
				.body(data.Meetingpayload(userPrincipalName,startDate,endDate));
	}
	...
	public void user_send_post_request_with_list_of_issues_assinee_status_api() {
		...
		response = reqspec.when().post("/teams/meeting/count").then().spec(resspec).log().all().extract().response();
